"""Mockup city built only from the road pieces and building kit (plus placeholder cars
for scale). Shadows are baked in by make_all.py."""
from assemble import building, roof_grid
from cpcgfx import Canvas, parse_art, TW, TH, RGB
...
# placeholder cars, only for scale (not part of the tile sheets)
CAR_N = parse_art("""
.RR.
RwwR
RRRR
...
def render_all(m, out_dir):
    img = m.image()
    # cars: taxi eastbound on the avenue, red car northbound on the west street,
    # teal car southbound on the avenue
    overlay(img, TAXI_E, 36 * TW + 2, 18 * TH + 38)
    overlay(img, TAXI_E, 6 * TW + 1, 18 * TH + 50)
    overlay(img, CAR_N, 10 * TW + 9, 26 * TH + 2)
    overlay(img, CAR_S, 28 * TW + 9, 13 * TH + 6)
    overlay(img, CAR_N, 30 * TW + 2, 23 * TH + 3)
