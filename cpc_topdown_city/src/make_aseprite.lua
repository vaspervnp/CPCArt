...
--   mode=frames : one 8x16 frame per tile, frame tags per group (tile sheet)
...
for line in io.lines(inpath) do
  ...
  elseif kind == "tag" then
    local n, a, b = rest:match("(%S+)%s+(%d+)%s+(%d+)")
    tags[#tags + 1] = { n, tonumber(a), tonumber(b) }
...
local function tile_image(hex)
  local img = Image(8, 16, ColorMode.INDEXED)
  for y = 0, 15 do
    for x = 0, 7 do
      local k = y * 8 + x + 1
